...
void TuiApp::refresh_selected_details() {
    ...
    bool pid_changed = (selected_pid != details_last_pid_);
    bool tab_changed = (dp.active_tab != details_last_tab_);
...
void TuiApp::render_details_panel() {
    ...
    int max_scroll = std::max(0, data_count - visible_details_rows_);
